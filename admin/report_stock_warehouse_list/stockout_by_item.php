...
<?php 
 if(isset($_POST['search'])){
  ...
  if($_POST['txt_option'] != ""){
    ...
    $sql = "SELECT * FROM tbl_pos_wh_stock_out AS A
          ...
          WHERE A.whso_date_record >= '$dateStart' AND A.whso_date_record <= '$dateEnd' AND B.pro_id='$pro_id_s'               
            ";  
    ...
    $sql = "SELECT * FROM tbl_pos_wh_stock_out AS A
                    LEFT JOIN tbl_pos_product AS B ON A.whso_product_code=B.pro_id
                    LEFT JOIN tbl_pos_employee AS C ON A.whso_employee=C.emp_id
          WHERE A.whso_date_record >= '$dateStart' AND A.whso_date_record <= '$dateEnd'               
            ";  
    $result = $connect->query($sql);
  }            
}else{
  ...
  $sql = "SELECT * FROM tbl_pos_wh_stock_out AS A
                    LEFT JOIN tbl_pos_product AS B ON A.whso_product_code=B.pro_id
                    LEFT JOIN tbl_pos_employee AS C ON A.whso_employee=C.emp_id
          WHERE A.whso_date_record='$v_date'
                          ";  
  $result = $connect->query($sql);
}
?>
...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-image fa-fw"></i> Report Warehouse Stockout by Item</h2>
        </div>
    </div>
    <br>
    <br>
            ...
            <form class="form-inline" method = "post" action="">
                ...
                <div class="form-group">
                    <select class="form-control" name = "txt_option"> 
                      ...
                      <?php 
                        $vv_search = $connect->query("SELECT * FROM tbl_pos_product ORDER BY code ASC");
                        while ($row_vv_search = mysqli_fetch_object($vv_search)) {
                          if($row_vv_search->pro_id == @$_POST['txt_option']){
                            echo '<option SELECTED value="'.$row_vv_search->pro_id.'">'.$row_vv_search->code.' :: '.$row_vv_search->ref.'</option>';
                          }else{
                            echo '<option value="'.$row_vv_search->pro_id.'">'.$row_vv_search->code.' :: '.$row_vv_search->ref.'</option>';
                          }
                        }
                       ?>
                    </select>
            ...
            <table class="table table-striped table-bordered table-hover dataTable dtr-inline " id="sample_2" role="grid" aria-describedby="sample_1_info">
              ...
              <tbody>
                  ...
                  $total=0;
                      ...
                      {     
                        $total+= $row["whso_qty_out"];

                        $v1=$i++;
                        $v2=$row["whso_date_record"];
                        $v3=$row["whso_letter_no"];
                        $v4=$row["code"];
                        $v5=$row["ref"];
                        $v6=$row["whso_qty_out"];
                        $v7=$row["whso_unit"];
